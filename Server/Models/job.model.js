...
// Töö lugemine
jobSchema.statics.read = async () => {
  return new Promise(async (resolve, reject) => {
    const jobs = await Job.find().populate("clientId").populate("serviceId")
...
jobSchema.statics.update = async ({
  ...
  return new Promise(async (resolve, reject) => {
    ...
    //muutmine
    const filter = { _id }
    const update = { description, unit, price, tax, status, total, dueDate, clientId: clientId._id, serviceId: serviceId._id }

    Job.findOneAndUpdate(filter, update, {
      returnDocument: "after",
    }).populate("clientId").populate("serviceId").then((updateJob) => {
      resolve(updateJob)
