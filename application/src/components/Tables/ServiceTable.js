...
import { Button, Dialog, DialogActions, DialogTitle, Input, Select, Snackbar, useSnackbar } from "@mui/material";
...
function ServiceTable(){
  ...
  const handleDeleteService = async () => {
    ...
    const taxEditor = (options) => {
        return (
            <Dropdown value={options.value} options={statuses} optionLabel="label" optionValue="value"
                onChange={(e) => options.editorCallback(e.value)} placeholder="Vali KM"
                itemTemplate={(option) => {
    ...
    const deleteRow = (rowData) => {
        return (
            <React.Fragment>
                <Primebutton icon="pi pi-trash" className="p-button-rounded p-button-danger p-button-text" onClick={() => confirmDeleteProduct(rowData)} />
            </React.Fragment>
        );
    }
    ...
    const priceEditor = (options) => {
        return <Input type="number" value={options.value} onChange={(e) => options.editorCallback(e.value)} mode="currency" currency="USD" locale="en-US" />
    }

    const priceBodyTemplate = (rowData) => {
        return new Intl.NumberFormat('en-EU', { style: 'currency', currency: 'EUR' }).format(rowData.price);
    }
    ...
    return (
        <div className="datatable-editing-demo">
            ...
            <div className="card p-fluid">
                ...
                sortField="code" sortOrder={1} resizableColumns  editMode="row" size='small' dataKey="id" showGridlines  stripedRows onRowEditComplete={onRowEditComplete1} responsiveLayout="scroll">
                    ...
                    <Column field="price" header="Hind" sortable body={priceBodyTemplate} editor={(options) => priceEditor(options)} style={{ width: '15%' }} resizeable={false}></Column>
                    <Column field="tax" header="KM" sortable body={statusBodyTemplate} editor={(options) => taxEditor(options)} style={{ width: '10%' }} resizeable={false}></Column>
                    <Column rowEditor headerStyle={{ width: '5%', borderRight: "none" }} bodyStyle={{ textAlign: 'right', borderRight: "none"}}></Column>
                    <Column body={deleteRow} headerStyle={{ width: '5%', borderLeft: "none" }} bodyStyle={{ textAlign: 'left', borderLeft: "none"}}></Column>
                </DataTable>
            </div>
      ...
      >
        <DialogTitle>
          {"Olete kindel, et soovite rida kustutada?"}
        </DialogTitle>
        <DialogActions>
